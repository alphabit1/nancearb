...
let triangle = {
  getPairs: () => {
    return new Promise((res, rej) => {
      const bRest = new api.BinanceRest({
        key: "KKPXP4aUm1C8Z5KaMrQTdWBqLl2H0VpMoVqg8gK1l0yvh2UumLoNn6Q8xQEyQVTK", // Get this from your account on binance.com
        secret:
          "ozy2jyZNw1N45PaTsp1m39zKV3UEnJ4j4Z7ILLDkZYExaF9AUH2jFoYzynJXJCJ9", // Same for this
      ...
      bRest
        ...
        .then((r1) => {
          ...
          r1.symbols.forEach((d) => {
            if (symbols.indexOf(d.baseAsset) === -1) {
              symbols.push(d.baseAsset);
            }
            if (symbols.indexOf(d.quoteAsset) === -1) {
              symbols.push(d.quoteAsset);
            }
            if (d.status === "TRADING") {
              validPairs.push(d.symbol);
              symValJ[d.symbol] = { bidPrice: 0, askPrice: 0 };
            }
          });
  ...
  startServer: () => {
    return new Promise((res, rej) => {
      ...
      server = app.listen(3000, () => {
        console.log(
          "Arbitrage Bot has just started on port 3000. Please wait....."
        );
      });
      app.use(cors());
  ...
  calculate: () => {
    console.log(
      'Finished SetUp. Open "http://127.0.0.1:3000/" in your browser to access. Happy Trading!!'
    );
